        ...
        <div className="flex flex-col">
          <div className="relative p-4 w-80 mx-auto md:mx-0 sm:w-96">
            ...
            <div className="flex w-80 sm:w-96 space-x-2 mx-auto sm:mx-0 justify-center">
              {images.map((image: any, index: number) => {
                return (
                    <a key={index} onClick={() => tumbHandle(index)} className={`flex rounded-2xl overflow-hidden cursor-pointer border-4 border-transparent ${currentIndex === index ? "border-yellow-600" : ""}`}>
                      <Image
                        alt="ss"
                        src={
          ...
          <div>
            ...
            <ul>
              {colors.map((color: string, idx: number) => {
                return (
                  <li key={idx} className="w-4 h-4 rounded-full" style={{backgroundColor: `${color}`}}>
                  </li>
                )
              })}
            </ul>
            <div className="flex space-x-5 lg:space-x-20">
              <div>
                <label htmlFor="qty" className="font-bold">QTY</label>
                <div className="inline ml-3 py-2 border rounded-3xl">
                  <button onClick={decreaseQtyNumber}>-</button>
                  <input id="qty" type="number" onChange={(e) => handleQtyInput(e)} value={inputQty} min="1" max={qty} step="1" className="w-5" />
